...
def getListings(conn):
    ...
    conn.request("GET","""/api/listings?filter={"_id":1}""")
    response = conn.getresponse()
    data = response.read()
    d = json.loads(data)

    # Array of user IDs
    listings = [str(d['data'][x]['_id']) for x in xrange(len(d['data']))]
...
def main(argv):
    ...
    while len(listings):
        ...
        for listing in listings:
            conn.request("DELETE","/api/listings/"+listing)
            response = conn.getresponse()
            data = response.read()
    ...
    # Exit gracefully
    conn.close()
    print "All users and Listings removed at "+baseurl+":"+str(port)
